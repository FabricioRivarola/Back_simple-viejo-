...
export const addMateria = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 1) {
      ...
      return res.status(500).json({ message: "No se creó la materia", success: false });
    }
  } catch (error) {
...
// cursar: Relacionar un usuario con la materia
export const cursar = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 1) {
      return res.status(200).json({ message: "Relación creada", success: true });
    } else {
      return res.status(500).json({ message: "No se creó la relación", success: false });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message, success: false });
  }
